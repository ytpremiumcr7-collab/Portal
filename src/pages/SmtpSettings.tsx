...
export default function SmtpSettings() {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="border-slate-200 bg-white shadow-sm"><CardHeader><CardTitle className="text-sm">Estado: {settings.data?.status ?? "…"}</CardTitle></CardHeader>
      ...
      <Card className="border-slate-200 bg-white shadow-sm"><CardHeader><CardTitle className="text-sm">Outbox reciente</CardTitle></CardHeader>
        <CardContent className="p-0 overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-slate-200 text-left text-xs text-slate-500"><th className="p-3">ID</th><th>Tipo</th><th>Estado</th><th>Intentos</th></tr></thead>
          <tbody>{(outbox.data?.items ?? []).map((r: any) => (
            <tr key={r.id} className="border-b border-slate-200"><td className="p-3">{r.id}</td><td>{r.eventType ?? r.aggregateType}</td><td>{r.status}</td><td>{r.attempts ?? "—"}</td></tr>
