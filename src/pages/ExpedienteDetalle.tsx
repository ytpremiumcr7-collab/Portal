...
const expLabels: Record<string,string> = { INTEGRACION:"Integración", REVISION_JURIDICA:"Revisión jurídica", APROBADO:"Aprobado", OBSERVADO:"Observado", CERRADO:"Cerrado", ARCHIVADO:"Archivado" };
...
export default function ExpedienteDetalle(){
  ...
  const item=(reqs.data as any)?.[0]?.expediente;
  return <div className="space-y-6">
    <div className="flex items-center gap-3"><Link to="/expedientes"><Button variant="ghost" size="icon" className="text-slate-400"><ArrowLeft className="w-5 h-5"/></Button><div><h2 className="text-2xl font-bold text-white">Expediente #{expedienteId}</h2><p className="text-sm text-slate-400">Requisitos, evidencia y cadena de eventos.</p></div></div>
