...
export const supplierAuthorities = mysqlTable("supplier_authorities", {
  ...
  authoritySource: mysqlEnum("supplier_authority_source", ["MIGRATED_LEGACY", "ADMIN_GRANTED", "VERIFIED_DOCUMENT"]).notNull(),
  scope: json("scope"),
...
export const submissionReceipts = mysqlTable("submission_receipts", {
  id: serial("id").primaryKey(),
  ...tenantColumns,
  receiptCode: varchar("receipt_code", { length: 80 }).notNull(),
  ...
  ciphertextHash: varchar("ciphertext_hash", { length: 64 }).notNull(),
  ...
  uniqueIndex("submission_receipt_code_uq").on(t.receiptCode),
